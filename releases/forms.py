...
class SelectWidgetBootstrap_tuple(forms.Select):
    """
    http://twitter.github.com/bootstrap/components.html#buttonDropdowns
    Needs bootstrap and jquery
    """
    ...
    def __init__(self, attrs={'class': 'btn-group noborder-fix main-identifier'}, choices=()):
        self.noscript_widget = forms.Select(attrs={}, choices=choices)
        super(SelectWidgetBootstrap_tuple, self).__init__(attrs, choices)

    def __setattr__(self, k, value):
        super(SelectWidgetBootstrap_tuple, self).__setattr__(k, value)
        if k != 'attrs':
            self.noscript_widget.__setattr__(k, value)
...
class ReleaseForm(forms.ModelForm):
    ...
    '''
    status = forms.ModelChoiceField(choices=Release.STATUSES,
